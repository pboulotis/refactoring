...
public class TaxpayerData extends JFrame {
  ...
  public TaxpayerData(int taxRegistrationNumber, TaxpayerManager taxpayerManager, FileManager fileManager) {
      ...
      public void mouseClicked(MouseEvent event) {
        ...
        try {
          if (event.getClickCount() == 1) {
            selectedReceiptId = Integer.valueOf(list.getSelectedValue().toString());
          }
        }catch(NullPointerException e1) {
          JOptionPane.showMessageDialog(null,
              "Please add a receipt on the list first");
        }
      }
    });
    ...
    btnAddReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        receiptID.setText(Integer.toString(currentReceiptId));
        receiptID.setToolTipText("Enter an identifying number, e.g. 1");
        JTextField date = new JTextField(16);
        date.setToolTipText("DD/MM/YYYY format, e.g. 1/1/2023");        
        JComboBox<String> kind = new JComboBox<String>(receiptKinds);
        JTextField amount = new JTextField(16);
        amount.setToolTipText("The charged amount number, e.g. 5.50");
        JTextField company = new JTextField(16);
        company.setToolTipText("The company's name, e.g. Wayne Enterprises");
        JTextField country = new JTextField(16);
        country.setToolTipText("The company's country of origin, e.g. USA");
        JTextField city = new JTextField(16);
        city.setToolTipText("The company's city, e.g. Gotham");
        JTextField street = new JTextField(16);
        street.setToolTipText("The company's street, e.g. North Wabash");
        JTextField number = new JTextField(16);
        number.setToolTipText("The company's street, e.g. 330");
        ...
        if (op == 0) {
          try {
            ...
            currentReceiptId = receiptIDValue + 1;
          } catch (IOException e1) {
            JOptionPane.showMessageDialog(null,
                "Problem with opening file ." + taxRegistrationNumber + "_INFO.txt");
            ...
            JOptionPane.showMessageDialog(null, "Please complete the form with all required information");
          }
        }
      }
    });
    ...
    btnDeleteReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if (selectedReceiptId != -1) {
          try {
            taxpayerManager.removeReceipt(selectedReceiptId);
          } catch (IOException e1) {
            JOptionPane.showMessageDialog(null,
                "Problem with opening file ." + taxRegistrationNumber + "_INFO.txt");
          } catch (WrongReceiptKindException e1) {
          ...
          receiptsModel.removeElement(selectedReceiptId);
        }
      }
    });
        ...
        if (answer == 0) {
          try {
            if (txtBox.isSelected()) {
              try {
                fileManager.saveLogFile(taxRegistrationNumber, "txt");
              } catch (IOException e1) {
                JOptionPane.showMessageDialog(null,
              ...
              try {
                fileManager.saveLogFile(taxRegistrationNumber, "xml");
              } catch (IOException e1) {
                JOptionPane.showMessageDialog(null,
    ...
    taxpayerStatus.setText(taxpayerManager.getTaxpayerStatusString(taxRegistrationNumber));
    ...
    editButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        JPanel receiptEditorPanel = new JPanel(new GridLayout(9, 2));
        JTextField receiptID = new JTextField(16);
        receiptID.setText(Integer.toString(currentReceiptId));
        receiptID.setToolTipText("Enter an identifying number, e.g. 1");
        JTextField date = new JTextField(16);
        ...
        JTextField number = new JTextField(16);
        number.setToolTipText("The company's street, e.g. 330");
        Receipt receipt = getSelectedReceipt(receipts, selectedReceiptId);
        
        receiptEditorPanel.add(new JLabel("Receipt ID:")); 
        receiptID.setText(Integer.toString(selectedReceiptId));
        receiptEditorPanel.add(receiptID);
        
        receiptEditorPanel.add(new JLabel("Date:"));
        date.setText(receipt.getIssueDate());
        receiptEditorPanel.add(date);
        
        receiptEditorPanel.add(new JLabel("Kind:"));
        receiptEditorPanel.add(kind);
        
        receiptEditorPanel.add(new JLabel("Amount:"));
        amount.setText(Float.toString(receipt.getAmount()));
        receiptEditorPanel.add(amount);
        
        receiptEditorPanel.add(new JLabel("Company:"));
        company.setText(receipt.getCompany().getName());
        receiptEditorPanel.add(company);
        
        receiptEditorPanel.add(new JLabel("Country:"));
        country.setText(receipt.getCompany().getCountry());
        receiptEditorPanel.add(country);
        
        receiptEditorPanel.add(new JLabel("City:"));
        city.setText(receipt.getCompany().getCity());
        receiptEditorPanel.add(city);
        
        receiptEditorPanel.add(new JLabel("Street:"));
        street.setText(receipt.getCompany().getStreet());
        receiptEditorPanel.add(street);
        
        receiptEditorPanel.add(new JLabel("Number:"));
        number.setText(Integer.toString(receipt.getCompany().getNumber()));
        receiptEditorPanel.add(number);

        int receiptIDValue, numberValue;
        float amountValue;
        String dateValue, kindValue, companyValue, countryValue;
        String cityValue, streetValue;
        int op = JOptionPane.showConfirmDialog(null, receiptEditorPanel, "",
            JOptionPane.OK_CANCEL_OPTION);
        if (op == 0) {
          try {
            receiptIDValue = Integer.parseInt(receiptID.getText());
    ...
    editButton.setBounds(260, 168, 89, 23);
    contentPane.add(editButton);
  }
  
  private Receipt getSelectedReceipt(HashMap<Integer, Receipt> receipts, int selectedId) {
    Iterator<HashMap.Entry<Integer, Receipt>> editIterator = receipts.entrySet().iterator();
    while (editIterator.hasNext()) {
      HashMap.Entry<Integer, Receipt> entry = editIterator.next();
      Receipt receipt = entry.getValue();
      if(selectedId == receipt.getId()) {
        return receipt;
      }
    }
    return null;
