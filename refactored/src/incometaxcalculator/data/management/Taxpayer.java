...
public class Taxpayer {
  ...
  private double [][] taxPercentages = {
      {0.0535, 0.0705, 0.0705, 0.0785, 0.0985},
      {0.0535, 0.0705, 0.0785, 0.0785, 0.0985},
      {0.0535, 0.0705, 0.0785, 0.0785, 0.0985},
      {0.0535, 0.0705, 0.0705, 0.0785, 0.0985}
  };
  private double [][] incomeLowerThanTresholds =
    ...
    {24680, 81080, 90000, 152540},    //Single 
  ...
  private double calculateBasicTaxByStatus(int status) {
    ...
    for (int j =0; j<incomeLowerThanTresholds[status].length; j++) {
      if (income < incomeLowerThanTresholds[status][j]) {
        return calculateBasicTaxProcess(status,j);
      }
    }
    return calculateBasicTaxProcess(status,4);
  }
  
  private double calculateBasicTaxProcess(int status, int percentage) {
    return baseTaxValues[status][percentage] + taxPercentages[status][percentage]
  ...
  public double getVariationTaxOnReceipts() {
    ...
    double[] receiptAmountToIncomePercentages =  {0.2, 0.4, 0.6};
    double[] variationTaxPercentages = {0.08, 0.04, -0.15};
    
    for (int i = 0; i < receiptAmountToIncomePercentages.length; i++) {
      if (totalAmountOfReceipts < receiptAmountToIncomePercentages[i] * income) {
  ...
  public String toString() {
    return "Taxpayer [fullname=" + fullname + ", taxRegistrationNumber=" + taxRegistrationNumber
        + ", income=" + income + ", taxpayerStatus=" + taxpayerStatus + "]";
  }
